document.addEventListener("DOMContentLoaded", () => {
  ...
  const QUIZ_DATA = {
    1: {
      q_num: "Q1",
      question: '人に"見せる作品"や"表現"をつくることが好き？',
      yes: "2",
      no: "7",
    },
    2: {
      q_num: "Q2",
      question: "最近何か新しいことに挑戦した？",
      yes: "3",
      no: "4",
    },
    3: {
      q_num: "Q3",
      question: "知らないことがあったらすぐに調べてしまう？",
      yes: "all_interested",
      no: "4",
    },
    4: {
      q_num: "Q4",
      question: "自分の作品は自分で操作できた方が楽しい？",
      yes: "1a",
      no: "5",
    },
    5: {
      q_num: "Q5",
      question: "その作品動画で見せたい？",
      yes: "6",
      no: "2a",
    },
    6: {
      q_num: "Q6",
      question: "2次元に行けるなら行きたい？",
      yes: "3a",
      no: "4a",
    },
    7: {
      q_num: "Q7",
      question: "回路図を見ているとニヤリとしてしまう？",
      yes: "6a",
      no: "8",
    },
    8: {
      q_num: "Q8",
      question: "サイバー空間で攻撃から守りたい？",
      yes: "5a",
      no: "9",
    },
    9: {
      q_num: "Q9",
      question: "人の知識を学んだコンピューターの頭脳を駆使したい？",
      yes: "7a",
      no: "10",
    },
    10: {
      q_num: "Q10",
      question: "スマホの中の世界を作ってみたい？",
      yes: "8a",
      no: "11",
    },
    11: {
      q_num: "Q11",
      question: "パソコンさんに命令をすることが好き？",
      yes: "9a",
      no: "cant_choose",
    },
    // 結果ノード
    "1a": {
      result: "あなたにおすすめの分野は...<br>",
      detail: "ゲーム",
      department: "ゲーム",
    },
    "2a": {
    ...
    "5a": {
      result: "あなたにおすすめの分野は...<br>",
      detail: "ネットワーク・セキュリティー",
      department: "ネットワーク・セキュリティー",
    },
    "6a": {
      result: "あなたにおすすめの分野は...<br>",
      detail: "電気・電子",
    ...
    // 特別な結果（TOPページの展示・出店リストに飛ぶ）
    all_interested: {
      result: "全部興味あるかも!?<br>",
      detail: "全部見にいこう！",
      department: "all_exhibitions",
      ...
      exhibitions: [
        {
          ...
          description:
            "絵コンテから動画までの制作過程を公開。学生の描いた貴重な原画も見られます！",
      ...
      exhibitions: [
        {
          shopName: "Webデザイン科",
          description:
            "学生の作品、ちょっとのぞいてみませんか？』<br><br>Webデザイン科では、学生たちが授業で取り組んできたWebサイトやWebページ、バナー広告、ランディングページなどの作品を展示しています。デザインの工夫や制作の裏側について、学生本人が直接ご紹介します。「どんな勉強をしているの？」「どんなスキルが身につくの？」といった疑問にも答えますので、ぜひ気軽にお立ち寄りください。",
  ...
  const accordionHeaderText = document.querySelector(
  ...
  const updateAccordion = (departmentName) => {
    ...
    accordionHeaderText.innerHTML = `<span class="result-department-in-header">${departmentName}</span>分野のあなたは この展示がおすすめ！`;
  ...
  const showNext = (id) => {
    ...
    // 結果 (IDに 'a' が含まれる場合、または特別な結果の場合)
    if (id.endsWith("a") || item.isSpecialResult) {
      // 特別な結果の場合（TOPページに飛ぶ）
      if (item.isSpecialResult) {
        // 結果文をセット（クリック可能にする）
        questionBox.innerHTML =
          item.result +
          `<span class="result-department clickable-result" data-target="exhibitions">${item.detail}</span>`;
        ...
        // 通常の結果の場合
        questionBox.innerHTML =
          item.result + `<span class="result-department">${item.detail}</span>`;
    ...
    // 質問
      ...
      // 質問番号と質問文をセット
      const questionNumber = item.q_num
        ? `<span class="quiz-number">${item.q_num}</span><br>`
        : "";
